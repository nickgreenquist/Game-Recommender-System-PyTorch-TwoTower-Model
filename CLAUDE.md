...
- **Use TaskCreate for multi-step work that fits one session, not text-form plans.** When a task has 3+ steps with their own verifications and you'll finish it in a single sitting (e.g. "implement Bucket N"), track it with TaskCreate/TaskUpdate so progress is visible in the UI. Don't also write the plan out as inline prose or a separate `.md` file — that's redundant.

- **For large features that span multiple sessions, the durable anchor is memory, not the task list.** The TaskCreate list is session-scoped — it starts empty in every new session and won't carry a plan across days. So a big feature (data loading → feature engineering → model retraining → A/B test on eval metrics → downstream changes) lives as a memory entry (loaded each session via MEMORY.md): keep the plan, the design decisions, and a one-line progress checkpoint there, and re-materialize the per-session TaskCreate list from it at the start of each working block. Cross-session state otherwise lives in its natural home — on-disk artifacts, git commits, and the user's reported metrics — not in a checked-in plan `.md`.
