...
def _build_synthetic_user_inputs(fs: dict, user_type: str, pad_idx: int):
    """
    Build the 5 user-side inputs for the ranker:
...
def _synthetic_user_history_tensors(full_ids: list, full_pw: list,
                                     device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Pack a synthetic user's history into B=1 tensors for the shared cross-feature
    utils. Returns (history_indices (1, L), history_weights (1, L)).
    """
...
def run_canary(cg_checkpoint: str | None = None,
    ...
    for user_type in canaries:
        ...
        with torch.no_grad():
            ...
            # Pack the synthetic history into (B=1, L) tensors and run the same
            ...
            if full_ids:
                hist_t, hist_w = _synthetic_user_history_tensors(full_ids, full_pw, device)
                cand_t1        = cand_t.unsqueeze(0)                                    # (1, n_cand)
                genre_ov = weighted_overlap(ranker.game_genre_binary, ranker.game_genre_count,
                                            history_indices=hist_t, history_weights=hist_w, cand_idx=cand_t1).squeeze(0)
